...
def startstop(command, minION):
    ...
    if OPER == "osx":
        p = os.popen(
            "/Applications/MinKNOW.app/Contents/Resources/bin/mk_manager_client -i "
            + minION
            + " --"
            + command,
            "r",
        ...
        p = os.popen(
            "C:\\grouper\\binaries\\bin\\mk_manager_client.exe -i "
            + minION
            + " --"
            + command,
            "r",
        ...
        p = os.popen(
            "/opt/ONT/MinKNOW/bin/mk_manager_client -i" + minION + " --" + command, "r"
        )
        while 1:
            line = p.readline()
            if not line:
...
def startrun(script, ipadd, port):
    try:
        startruncustom = (
            '{"id":1, "method":"start_script","params":{"name":"' + script + '"}}'
        )
        startresult = execute_command_as_string(startruncustom, ipadd, port)
...
def stoprun_message(port, ipadd, message):
    try:
        stopresult = execute_command_as_string(commands("stoprun"), ipadd, port)
        stopprotocolresult = execute_command_as_string(
            commands("stopprotocol"), ipadd, port
        )
...
def renameflowcell(name, ipadd, port):
    try:
        set_flowcell_id = (
            '{"id":1,"method":"set_engine_state","params":{"state_id":"flow_cell_id","value":"'
            + name
            + '"}}'
        )
        set_sample_id_result = execute_command_as_string(set_flowcell_id, ipadd, port)
        startresultmessage = send_message_port(
            "minoTour renamed the flowcell to " + name, ipadd, port
        )
    except Exception as err:
        print >>sys.stderr, err
...
def renamerun(name, ipadd, port):
    try:
        ...
        set_sample_id_result = execute_command_as_string(set_sample_id, ipadd, port)
        startresultmessage = send_message_port(
            "minoTour renamed the run to " + name, ipadd, port
        )
    except Exception as err:
        print >>sys.stderr, err
        ...
        if not m.is_binary:
            if hasattr(self, "minion"):
                try:
                    ...
                    self.minIONdict[self.minion]["APIHelp"].update_message(
                        json.loads(str(m))
                    )
                except Exception as err:
                    print(err)
...
def get_run_scripts(ipadd, port):
    get_scripts = '{"id":"1", "method":"get_script_info_list","params":null}'
            ...
            if "livedata" in minIONdict[minion]:
                if "yield_res" in minIONdict[minion]["livedata"]:
                    if "result" in minIONdict[minion]["livedata"]["yield_res"]:
                        try:
                            asictemp = float(
                                minIONdict[minion]["detailsdata"]["engine_states"][
                                    "minion_asic_temperature"
                                ]
                            )
                        except:
                            asictemp = 0
                        try:
                            heatsinktemp = float(
                                minIONdict[minion]["detailsdata"]["engine_states"][
                                    "minion_heatsink_temperature"
                                ]
                            )
                        except:
                            heatsinktemp = 0
                        if (
                            minIONdict[minion]["livedata"]["bias_voltage_gain"][
                                "result"
                            ]
                            != "null"
                        ):
                            ...
                            biasvoltage = 0
                        if (
                            minIONdict[minion]["livedata"]["biasvoltageget"]["result"]
                            != "null"
                        ):
                            voltage_val = int(
                                minIONdict[minion]["livedata"]["biasvoltageget"][
                                    "result"
                                ]["bias_voltage"]
                            )
                        else:
                        ...
                        try:
                            if (
                                minIONdict[minion]["livedata"]["yield_res"]["result"]
                                != "null"
                            ):
                                yieldval = int(
                                    minIONdict[minion]["livedata"]["yield_res"][
                                        "result"
                                    ]
                                )
                            else:
                                yieldval = minIONdict[minion]["detailsdata"][
                                    "statistics"
                                ]["read_event_count"]
                        except:
                            yieldval = 0
                        ...
                        if (
                            "meanratio_history"
                            not in minIONdict[minion]["pore_history"]
                        ):
                        ...
                        if "simplesummary" in minIONdict[minion]:
                            if "strand" in minIONdict[minion]["simplesummary"].keys():
                                strand = float(
                                    minIONdict[minion]["simplesummary"]["strand"]
                                )
                            if (
                                "single_pore"
                                in minIONdict[minion]["simplesummary"].keys()
                            ):
                                single_pore = float(
                                    minIONdict[minion]["simplesummary"]["single_pore"]
                                )
                            elif (
                                "good_single"
                                in minIONdict[minion]["simplesummary"].keys()
                                ...
                                single_pore = float(
                                    minIONdict[minion]["simplesummary"]["good_single"]
                                )
                        ...
                        if len(minIONdict[minion]["yield_history"]) > 1:
                            if yieldval > minIONdict[minion]["yield_history"][-1][1]:
                                minIONdict[minion]["yield_history"].append(
                                    (
                                        minIONdict[minion]["detailsdata"]["timestamp"]
                                        * 1000,
                                        yieldval,
                                    )
                                )
                            minIONdict[minion]["temp_history"]["asictemp"].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                                    * 1000,
                                    asictemp,
                            ...
                            minIONdict[minion]["temp_history"]["voltage"].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                                    * 1000,
                                    voltage_value,
                            ...
                            minIONdict[minion]["temp_history"]["heatsinktemp"].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                                    * 1000,
                                    heatsinktemp,
                            ...
                            minIONdict[minion]["pore_history"]["strand"].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                                    * 1000,
                                    strand,
                            ...
                            minIONdict[minion]["pore_history"]["percent"].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                                    * 1000,
                                    percent,
                            ...
                            minIONdict[minion]["pore_history"]["single"].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                                    * 1000,
                                    single_pore,
                            ...
                            minIONdict[minion]["pore_history"][
                                "meanratio_history"
                            ].append(
                                (
                                    minIONdict[minion]["detailsdata"]["timestamp"]
                            ...
                            if "simplesummary" in minIONdict[minion]:
                                if (
                                    "details"
                                    not in minIONdict[minion]["pore_history"].keys()
                                ):
                                    minIONdict[minion]["pore_history"][
                                        "details"
                                    ] = dict()
                                for state in minIONdict[minion]["simplesummary"].keys():
                                    if (
                                        ...
                                        minIONdict[minion]["pore_history"]["details"][
                                            state
                                        ] = []
                                    minIONdict[minion]["pore_history"]["details"][
                                        state
                                        ...
                                        (
                                            ...
                                            * 1000,
                                            minIONdict[minion]["simplesummary"][state],
                                        )
                                    )
                        else:
                            minIONdict[minion]["yield_history"].append(
            ...
            if "channelstuff" in minIONdict[minion]:
                for item in minIONdict[minion]["channelstuff"]:
                    if "style" in minIONdict[minion]["channelstuff"][item]:
                        colourlookup[
                            minIONdict[minion]["channelstuff"][item]["style"]["label"]
                        ] = minIONdict[minion]["channelstuff"][item]["style"]["colour"]
                if "cust_chan_dict" not in minIONdict[minion]:
                    minIONdict[minion]["cust_chan_dict"] = dict()
                    for i in chanlookup_type(minion):
                        minIONdict[minion]["cust_chan_dict"][i] = dict()
                for channel in minIONdict[minion]["detailsdata"]["channel_info"][
                    "channels"
                ]:
                    ...
                    if "state_group" in channel:
                        if channel["state"] in colourlookup:
                            statedict[minion].update(
                                {
                                    channel["name"]: {
                                        "state": channel["state"],
                                        "state_group": channel["state_group"],
                                        "colour": colourlookup[channel["state"]],
                                    }
                                }
                            )
                            minIONdict[minion]["cust_chan_dict"][
                                int(channel["name"])
                            ] = channel["state"]
                            ...
                            statedict[minion].update(
                                {
                                    channel["name"]: {
                                        "state": channel["state"],
                                        "state_group": channel["state_group"],
                                        "colour": colourlookup[channel["state_group"]],
                                    }
                                }
                            )
                            minIONdict[minion]["cust_chan_dict"][
...
class DummyClient(WebSocketClient):
    ...
    def opened(self):
        self.send(
            json.dumps(
                {"engine_states": "1", "channel_states": "1", "channel_info": "1"}
            )
        )
            ...
            for element in json_object:
                if element == "statistics" and json_object[element] != "null":
                    for element2 in json_object[element]:
                        if (
                            element2 != "read_statistics"
                            and element2 != "channel_info"
                            and json_object[element][element2] != "null"
                        ):  # and element2 != "read_statistics" and element2 != "completed_samples":
                            if element not in self.detailsdict:
                                self.detailsdict[element] = dict()
                            self.detailsdict[element][element2] = json_object[element][
                                element2
                            ]
                    ...
                    for element2 in json_object[element]:
                        if json_object[element][element2] != "null":
                            ...
                            if json_object[element][element2] is not dict:
                                self.detailsdict[element][element2] = json_object[
                                    element
                                ][element2]
                    if "status" in json_object[element]:
                        ...
                        if json_object[element]["status"] == "starting":
                            ...
                            if self.ejecteject == 0:
                                if (
                                    self.minion in self.minIONdict
                                    and "yield_history" in self.minIONdict[self.minion]
                                ):
                                    self.minIONdict[self.minion]["yield_history"] = []
                                    self.minIONdict[self.minion]["temp_history"][
                                        "asictemp"
                                    ] = []
                                    self.minIONdict[self.minion]["temp_history"][
                                        "heatsinktemp"
                                    ...
                                    self.minIONdict[self.minion]["pore_history"][
                                        "strand"
                                    ] = []
                                    self.minIONdict[self.minion]["pore_history"][
                                        "percent"
                                    ...
                                    if (
                                        "details"
                                        in self.minIONdict[self.minion]["pore_history"]
                                    ):
                                        self.minIONdict[self.minion][
                                            "pore_history"
                                        ].pop("details")
                        if (
                            json_object[element]["status"] == "processing"
                            and self.ejecteject == 0
                        ):
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    self.minIONdict[self.minion]["pore_history"].pop(
                                        "details"
                                    )
                            self.ejecteject = 1
...
class MinControlAPI:
    ...
    def __init__(
        self, minion_name, args, statedict, summarystatedict, minIONdict, header
    ):
    ...
    def check_jobs(self, minION):
        ...
        for job in jobs:
            ...
            if job["job"] == "testmessage":
                ...
                r = requests.post(
                    self.minion["url"] + "control/" + str(job["id"]) + "/",
                    headers=self.header,
                )
                # print (r.text)
    ...
    def create_run(self, runid):
        ...
        if not run:
            ...
            #
            flowcell = self.minotourapi.get_flowcell_by_name(
                self.status_summary["flow_cell_id"]
            )

            if not flowcell:

                flowcell = self.minotourapi.create_flowcell(
                    self.status_summary["flow_cell_id"]
                )
    ...
    def update_minion_run_stats(self):
        ...
        requests.post(self.runidlink + "rundetails/", headers=self.header, json=payload)
    ...
    def update_message(self, messagestring):
        ...
        # print (payload)
        createminIONStatus = requests.post(
            self.minion["url"] + "messages/", headers=self.header, json=payload
        )
    ...
    def update_minion_current_stats(
        self, livedata, detailsdata, simplesummary, channelstuff
    ):
        try:
            if self.args.GUI:
                self.args.minKNOWmessage = livedata["status"]["result"]
                self.args.minKNOWreadsseen = livedata["get_seq_metrics"]["result"][
            ...
            self.status_summary["sample_name"] = livedata["get_engine_states"][
                "result"
            ]["sample_id"]
            self.status_summary["minKNOW_version"] = livedata["get_engine_states"][
                "result"
            ]["version_string"]
            self.status_summary["flow_cell_id"] = livedata["get_engine_states"][
                "result"
            ]["flow_cell_id"]
            self.status_summary["run_name"] = livedata["get_engine_states"]["result"][
                "data_set"
            ]
            self.status_summary["hash_run_id"] = livedata["get_engine_states"][
                "result"
            ]["hash_run_id"]
            self.status_summary["exp_script_purpose"] = livedata["get_engine_states"][
                "result"
            ]["exp_script_purpose"]
            self.status_summary["script_run_id"] = livedata["get_engine_states"][
                "result"
            ]["script_run_id"]
            self.status_summary["real_sample_rate"] = livedata["get_engine_states"][
                "result"
            ]["real_sample_rate"]
            self.status_summary["asic_id"] = livedata["get_engine_states"]["result"][
                "asic_id"
            ]
            self.status_summary["bytes_capacity"] = livedata["disk_space"]["result"][0][
                "bytes_capacity"
            ]
            self.status_summary["bytes_available"] = livedata["disk_space"]["result"][
                0
            ]["bytes_available"]
            self.status_summary["bytes_when_alert_issued"] = livedata["disk_space"][
                "result"
            ][0]["bytes_when_alert_issued"]
            self.status_summary["recommend_alert"] = livedata["disk_space"]["result"][
                0
            ]["recommend_alert"]
            self.status_summary["histogram"] = str(
                livedata["get_seq_metrics"]["result"]["read_event_count_weighted_hist"]
            )
            self.status_summary["bin_width"] = livedata["get_seq_metrics"]["result"][
                "read_event_count_weighted_hist_bin_width"
            ]
            self.status_summary["read_count"] = livedata["get_seq_metrics"]["result"][
                "selected_completed_count"
            ]
            ...
            payload = {
                ...
                "minKNOW_disk_space_till_shutdown": self.status_summary[
            ...
            if self.minstatexist is False:
                ...
                if r.status_code == 200:
                    ...
                    createminIONStatus = requests.post(
                        self.minion["url"] + "status/",
                        headers=self.header,
                        json=payload,
                    )
            else:
                createminIONStatus = requests.put(
                    self.minion["url"] + "status/", headers=self.header, json=payload
                )
    ...
    def update_minion_stats(self, livedata, detailsdata, simplesummary):
        if "yield_res" in livedata:
            if "result" in livedata["yield_res"]:
                try:
                    asictemp = float(
                        detailsdata["engine_states"]["minion_asic_temperature"]
                    )
                except:
                    asictemp = 0
                try:
                ...
                if livedata["biasvoltageget"]["result"] != "null":
                    voltage_val = int(
                        livedata["biasvoltageget"]["result"]["bias_voltage"]
                    )
                else:
                    voltage_val = 0
                voltage_value = voltage_val * biasvoltage
                ...
                try:
                    if livedata["yield_res"]["result"] != "null":
                        ...
                        if self.args.GUI:
                            self.args.minKNOWeventsseen = int(
                                livedata["yield_res"]["result"]
                            )

                    else:
                        yieldval = detailsdata["statistics"]["read_event_count"]
                        if self.args.GUI:
                            self.args.minKNOWeventsseen = detailsdata["statistics"][
                                "read_event_count"
                            ]
                ...
                payload = {
                    ...
                    "event_yield": int(
                        livedata["get_seq_metrics"]["result"]["read_event_count"]
                    ),
                    "asic_temp": asictemp,
                ...
                # print (payload)
                createminionstat = requests.post(
                    self.runidlink + "runstats/", headers=self.header, json=payload
                )
    ...
    def update_minion_events(self, minION, computer, status):
        ...
        updatestatus = requests.post(
            self.minion["url"] + "events/", headers=self.header, json=payload
        )
    ...
    def update_script(self, minION, script):
        ...
        if script["name"] not in self.scripts:
            ...
            updatestate = requests.post(
                self.minion["url"] + "scripts/", json=payload, headers=self.header
            )
        else:
            ...
            if str(script["identifier"]) not in self.scripts:
                ...
                for element in json.loads(self.scripts):
                    if script["name"] in str(element):
                        ...
                        payload["name"] = script["name"]
                        if "experiment type" in script["tags"].keys():
                            payload["experiment_type"] = script["tags"][
                                "experiment type"
                            ]
                        if "flow cell" in script["tags"].keys():
                            payload["flow_cell"] = script["tags"]["flow cell"]
                        ...
                        if "kit" in script["tags"].keys():
                            payload["kit"] = script["tags"]["kit"]
                        update = requests.put(
                            self.minion["url"] + "scripts/" + str(element["id"]) + "/",
                            json=payload,
                            headers=self.header,
                        )
                        # print (update.text)
...
#                         self.minIONdict[deviceid]["APIHelp"] = MinControlAPI(
#                             deviceid,
#                             self.args,
#                             self.statedict,
#                             self.summarystatedict,
#                             self.minIONdict,
#                             self.header,
#                         )
#                         self.minIONdict[deviceid]["APIHelp"].update_minion_events(
#                             deviceid, "UNKNOWN", "connected"
#                         )
...
#                             ] = ws_longpoll_port
#                             self.minIONdict[deviceid][
#                                 "ws_event_sampler_port"
#                             ] = ws_event_sampler_port
#                             self.minIONdict[deviceid][
#                                 "ws_raw_data_sampler_port"
#                             ] = ws_raw_data_sampler_port
#                             # print ("port is:", port)
#                             results = execute_command_as_string(
#                                 commands("machine_id"),
#                                 self.args.ip,
...
#
#                             self.minIONdict[deviceid]["APIHelp"].update_minion_events(
#                                 deviceid, str(results["result"]), "active"
#                             )
#                         except:
#                             # print ("caught it")
#                             minIONports = list(
#                                 map(
#                                     lambda x: x - 192 + 8000 + 128,
#                                     filter(lambda x: x > 120, map(ord, thing)),
#                                 )
#                             )
#                             # print (minIONports)
#                             self.minIONdict[deviceid]["state"] = "active"
...
#                 # print (minIONdict)
#                 if (
#                     self.minIONdict == self.minIONdict_test
#                 ):  ## The dictionary is unchanged since the last cycle
#                     active = 0
#                     inactive = 0
...
#                                     + self.args.ip
#                                     + ":"
#                                     + str(self.minIONdict[minION]["ws_longpoll_port"])
#                                     + "/"
#                                 )
#                                 self.minIONclassdict[minION]["class"] = DummyClient(
#                                     connectip
#                                 )
#                                 connectip2 = (
#                                     "ws://"
...
#                                     + "/user_messages"
#                                 )
#                                 self.minIONclassdict[minION]["class2"] = MessagesClient(
#                                     connectip2, self.args
#                                 )
...
#                                         self.minIONclassdict[minION][
#                                             "class"
#                                         ].init_minion(minION, self.minIONdict)
#                                         print(
#                                             "Trying to init a messages client with {}".format(
#                                                 minION
#                                             )
#                                         )
#                                         self.minIONclassdict[minION][
...
#                                         ].init_minion(minION)
#                                         results = execute_command_as_string(
#                                             commands("get_analysis_configuration"),
#                                             self.args.ip,
#                                             self.minIONdict[minION]["port"],
#                                         )
#                                         # print ("getting the good stuff")
#
#                                         self.minIONdict[minION][
#                                             "channelstuff"
#                                         ] = results["result"]["channel_states"]
...
#                                     if (
#                                         self.minIONdict[minION]["state"]
#                                         == self.minIONdict_test[minION]["state"]
#                                     ):
#                                         try:  # To catch mysterious bugs
#                                             livedata = (
#                                                 dict()
#                                             )  # collect a dictionary of useful data - might change this
#                                             for query in (
#                                                 "status",
#                                                 "dataset",
#                                                 "biasvoltageget",
...
#                                                 "get_seq_metrics",
#                                                 "get_engine_states",
#                                             ):  # ,'getstaticdata','get_analysis_configuration'):
#                                                 results = execute_command_as_string(
#                                                     commands(query),
#                                                     self.args.ip,
#                                                     self.minIONdict[minION]["port"],
...
#                                             self.minIONdict[minION][
#                                                 "livedata"
#                                             ] = livedata  # append results to data stream
#                                             # print ("data appended")
#                                             # sys.exit()
#                                         except Exception as err:
#                                             print("line 1407", err)
...
#                                                 self.minIONdict[minION][
#                                                     "APIHelp"
#                                                 ].check_scripts(minION)
#                                                 for script in self.minIONdict[minION][
#                                                     "scripts"
#                                                 ]:
#                                                     # print (script)
#                                                     self.minIONdict[minION][
#                                                         "APIHelp"
#                                                     ].update_script(minION, script)
#                                         else:
...
#                                         "scripts"
#                                     ] = get_run_scripts(
#                                         self.args.ip, self.minIONdict[minION]["port"]
#                                     )
#                                     # print("calling check scripts 1161")
#                                     self.minIONdict[minION]["APIHelp"].check_scripts(
#                                         minION
#                                     )
...
#                             self.minIONdict, self.statedict, self.summarystatedict
#                         )
#                         self.minIONdict[minION]["APIHelp"].update_minion_current_stats(
#                             self.minIONdict[minION]["livedata"],
...
#                     ) in (
#                         self.minIONdict
#                     ):  # if yes then we check each minION to see what is happening
#                         if "state" in self.minIONdict[minION].keys():
#                             if (
#                                 self.minIONdict[minION]["state"] == "active"
#                             ):  # we have a sequencing minION - what is it doing?
#                                 # APIHelp =
#                                 # print (minION, "ACTIVE")
#                                 try:
#                                     if minION not in self.minIONclassdict:
...
#                                             )
#                                             + "/"
#                                         )  # Connect to the self.minIONdict
#                                         connectip2 = (
#                                             "ws://"
#                                             + self.args.ip
#                                             + ":"
...
#                                             + "/user_messages"
#                                         )  # Connect to the self.minIONdict
#                                         self.minIONclassdict[
#                                             minION
#                                         ] = dict()  # Add minION to the dictionary
#                                     if "connected" not in self.minIONclassdict[minION]:
#                                         # print ("Connecting to this minION", minION)
#                                         self.minIONclassdict[minION][
#                                             "class"
#                                         ] = DummyClient(connectip)
#                                         self.minIONclassdict[minION][
#                                             "class2"
#                                         ] = MessagesClient(
#                                             connectip2, self.args, self.minIONdict
#                                         )
#                                     if "connected" not in self.minIONclassdict[minION]:
#                                         try:
#                                             self.minIONclassdict[minION][
...
#                                             ].init_minion(minION, self.minIONdict)
#                                             print(
#                                                 "Trying to set the minion Again.... {}".format(
#                                                     minION
#                                                 )
#                                             )
#                                             self.minIONclassdict[minION][
...
#                                         "stats_for_channel_name"
#                                     ]:
#                                         if (
#                                             float(
#                                                 results["result"][
#                                                     "stats_for_channel_name"
#                                                 ][item]["seq_strand_delta"]
#                                             )
#                                             != 0
#                                         ):
#                                             meanratio.append(
...
#                                             )
#
#                                     results2 = execute_command_as_string(
#                                         commands("get_analysis_configuration"),
#                                         self.args.ip,
#                                         self.minIONdict[minION]["port"],
#                                     )
#                                     self.minIONdict[minION]["channelstuff"] = results2[
#                                         "result"
#                                     ]["channel_states"]
#                                     self.minIONdict[minION][
#                                         "livedata"
...
#                                         for element in self.minIONclassdict[minION][
#                                             "class"
#                                         ].detailsdict:
#                                             if (
#                                                 "detailsdata"
#                                                 not in self.minIONdict[minION]
#                                             ):
...
#                                             if (
#                                                 element == "statistics"
#                                                 and self.minIONclassdict[minION][
#                                                     "class"
#                                                 ].detailsdict[element]
#                                                 != "null"
#                                             ):
...
#                                                 ] = context
#                                             if (
#                                                 element == "engine_states"
#                                                 and self.minIONclassdict[minION][
#                                                     "class"
#                                                 ].detailsdict[element]
#                                                 != "null"
#                                             ):
#                                                 for element2 in self.minIONclassdict[
#                                                     minION
#                                                 ]["class"].detailsdict[element]:
#                                                     if (
#                                                         self.minIONclassdict[minION][
#                                                             "class"
#                                                         ].detailsdict[element][element2]
#                                                         != "null"
#                                                     ):
#                                                         if (
#                                                             element
#                                                             not in self.minIONdict[
#                                                                 minION
#                                                             ]["detailsdata"]
#                                                         ):
#                                                             self.minIONdict[minION][
...
#                                         print("<<< Exception")
#
#                                     self.minIONdict[minION]["APIHelp"].check_jobs(
#                                         minION
#                                     )
#
...
#             self.minIONdict[minION]["APIHelp"].update_minion_events(
#                 minION, "UNKNOWN", "unplugged"
#             )
#         print("bye bye")
