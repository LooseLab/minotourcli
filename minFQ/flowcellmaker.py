                ...
                if file.split("_read_")[0] != last_seen:
                    ...
                    try:
                        try:
                            ...
                            mymultifile = multi_fast5.MultiFast5File(
                        ...
                        if run_id not in flowcelldict[sample_id][flowcell].keys():
                            ...
                            flowcelldict[sample_id][flowcell][run_id][
                                "tracking_id"
                            ] = myfile.get_tracking_id()
                            flowcelldict[sample_id][flowcell][run_id][
                                "context_tags"
                            ] = myfile.get_context_tags()
                    except:
            ...
            for run_id in flowcelldict[sample_id][flowcellid]:
                ...
                if not run:
                    ...
                    run = minotourapi.create_run(
                        ...
                        flowcell["data"],
                        minion,
                        flowcelldict[sample_id][flowcellid][run_id]["tracking_id"][
                            "exp_start_time"
                        ],
                    )

                minion = minotourapi.get_minion_by_name(
                    flowcelldict[sample_id][flowcellid][run_id]["tracking_id"][
                        "device_id"
                    ]
                )
                ...
                try:
                    sample_rate = int(
                        str(
                            flowcelldict[sample_id][flowcellid][run_id]["context_tags"][
                                "sample_frequency"
                            ]
                        )
                    )
                ...
                payload = {
                    ...
                    "minKNOW_asic_id": flowcelldict[sample_id][flowcellid][run_id][
                        "tracking_id"
                    ]["asic_id"],
                    "minKNOW_start_time": flowcelldict[sample_id][flowcellid][run_id][
                        "tracking_id"
                    ]["exp_start_time"],
                    "minKNOW_computer": str(
                        flowcelldict[sample_id][flowcellid][run_id]["tracking_id"][
                            "hostname"
                        ]
                    ),
